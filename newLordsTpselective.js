(function() {
    ...
    // 3. UPGRADED LOGIC ENGINE (v3.0 Quant/Probability Scanner)
    function getDynamicPrediction(data) {
        ...
        // Look for 5, 4, 3, and 2 depth patterns
        for (let d of [5, 4, 3, 2]) {
            const seq = historyStr.slice(-d);
            let a_count = 0;
            let b_count = 0;
            
            // Scan the entire database for every occurrence of this sequence
            ...
            // Only act if the sequence has happened at least twice before to establish a baseline
            if (total >= 2) { 
                const a_prob = a_count / total;
                const b_prob = b_count / total;
                
                // STRICT TRIGGER: We only bet if the historical probability is 66%+
        ...
        // PHASE B: STRICT TECHNICAL INDICATORS (If no strong probability exists)
        ...
        // If we reach here, the table is a pure 50/50 coin flip. Do not bet.
    ...
    function updateEngineUI() {
        ...
        intelEl.innerHTML = `
            <div style="display:flex;justify-content:space-between;border-bottom:1px solid #333;padding-bottom:5px;margin-bottom:8px;">
                <b style="color:#0ff;">1-DAY TP MASTER (V3.0 SNIPER)</b>
        ...
        recentRounds.forEach(d => {
            ...
            if (!alreadyLogged) {
                ...
                if (window.tpLogs.length >= 10) {
                    ...
                    // ONLY log stats if the bot actually placed a bet (didn't sit out)
    ...
    function processGameData(d) {
        ...
        if (cardsArray.length === 6 && !cardsArray.includes("1")) {
            ...
            if (!alreadyLogged) {
                ...
                // ONLY update actual live stats if the bot placed a bet
                if (activePrediction && activePrediction.win !== "WAIT") {
                    if (winner === "TIE") {
                        window.tpStats.losses++; window.tpStats.units -= 1.0;
                        window.tpStats.streak = Math.min(-1, window.tpStats.streak - 1);
    ...
    updateEngineUI();
    console.log("%c MASTER 1-DAY TP SUITE ONLINE | V3.0 QUANT SNIPER ACTIVE ", "background:#000; color:#0f0; font-weight:bold; font-size:14px;");
})();