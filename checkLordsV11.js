(function () {
    ...
    function updateEngineUI() {
        ...
        intelEl.innerHTML = `
            <div style="display:flex;justify-content:space-between;border-bottom:1px solid #333;padding-bottom:5px;margin-bottom:8px;">
                <b style="color:#0ff;">V11.2 LORDS (TRACKING)</b>
    ...
    // 6A. HISTORY PROCESSOR (V2 ARCHITECTURE + V11.2 MATH)
    function processHistoryArray(historyArray) {
        ...
        recentRounds.forEach(d => {
            ...
            if (!alreadyLogged) {
                ...
                // Backtest Math against history BEFORE this hand is added
                if (window.tpLogs.length >= 25 && winner !== "TIE") {
                    ...
                    // Update memory banks for the next hand
    ...
    // 6B. LIVE ROUND PROCESSOR (V2 ARCHITECTURE + V11.2 MATH)
    function processGameData(d) {
        ...
        if (cardsArray.length === 6 && !cardsArray.includes("1")) {
            ...
            if (!alreadyLogged) {
                ...
                if (activePrediction && activePrediction.win !== "WAIT" && winner !== "TIE") {
    ...
    // 8. NEW NETWORK HOOKS (V2 ARCHITECTURE)
    ...
    btn.onclick = (e) => {
        ...
        link.download = `lords_v11_2_highvol_${Date.now()}.csv`;
        link.click();
    };
    ...
    // Initial Backfill Call to process existing localstorage data
    if (window.tpLogs.length > 0) {
        ...
        rawLogs.forEach(r => {
            processHistoryArray([{ mid: r.mid, win: r.winner === "A" ? "1" : (r.winner === "B" ? "2" : "0") }]);
        });
    }

    updateEngineUI();
    console.log("%c V11.2 LORDS HIGH-VOL FUSED WITH V2 ARCHITECTURE ", "background:#000; color:#0f0; font-weight:bold; font-size:14px;");
})();
...
(function () {
    ...
    // 8. DEEP JSON SCANNER (V2 ARCHITECTURE)
    function parseAndRoute(rawText) {
        if (!rawText) return;
        let payload;
        try { payload = JSON.parse(rawText); } catch(e) { return; }
    ...
    // 9. NETWORK HOOKS (V2 ARCHITECTURE)
    const originalFetch = window.fetch;
    window.fetch = async function(...args) {
        const response = await originalFetch.apply(this, args);
        try {
    ...
    updateEngineUI();
    console.log("%c V11.2 LORDS HIGH-VOL FUSED AUTO-BET ONLINE ", "background:#000; color:#0f0; font-weight:bold; font-size:14px;");
})();