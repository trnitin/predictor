(function() {
    ...
    function runSuite() {
        ...
        // 4. LOGIC ENGINE (V2.5 Smart Recency)
        function getDynamicPrediction(data) {
            ...
            // LAYER 1: ELITE DEEP PATTERNS
            for (let d of [5, 4]) {
                const seq = historyStr.slice(-d);
                const firstOccur = historyStr.lastIndexOf(seq, historyStr.length - (d + 1));
                if (firstOccur !== -1) {
                    const pred = historyStr[firstOccur + d];
                    if (pred) return { win: pred, reason: `ELITE PATTERN (${d} MATCH)` };
                }
            }

            // LAYER 2: MACRO TREND OVERRIDE
            if (aCount >= 9) return { win: "A", reason: "MACRO TREND OVERRIDE (A)" };
            if (bCount >= 9) return { win: "B", reason: "MACRO TREND OVERRIDE (B)" };

            // LAYER 3: STANDARD PATTERN
            const seq3 = historyStr.slice(-3);
            const occur3 = historyStr.lastIndexOf(seq3, historyStr.length - 4);
            if (occur3 !== -1) {
                const pred = historyStr[occur3 + 3];
                if (pred) return { win: pred, reason: `STANDARD PATTERN (3 MATCH)` };
            }

            // LAYER 4: SMART FALLBACKS
            const isChop = last4 === "ABAB" || last4 === "BABA";
            if (isChop) {
                return { win: lastWinner === "A" ? "B" : "A", reason: "CONFIRMED CHOP" };
            } 
            
            if (last3 === "AAA" || last3 === "BBB") {
                return { win: lastWinner, reason: "STREAK RIDER" };
            }

            if (aCount >= 7) return { win: "A", reason: "HEAVY A BIAS" };
            if (bCount >= 7) return { win: "B", reason: "HEAVY B BIAS" };

            return { win: lastWinner, reason: "MOMENTUM DEFAULT" };
        }
        ...
        function updateEngineUI() {
            ...
            intelEl.innerHTML = `
                ...
                <div style="display:flex;justify-content:space-between;border-bottom:1px solid #333;padding-bottom:5px;margin-bottom:8px;">
                    <b style="color:#0ff;">SHANKARA TP (V2.5)</b>
        ...
        updateEngineUI();
        console.log("%c SHANKARA TP SUITE ONLINE | V2.5 LOGIC & AUTO-DECRYPTION ENGAGED ", "background:#000; color:#0f0; font-weight:bold; font-size:14px;");
    }
})();
...
(function() {
    ...
    function runSuite() {
        ...
        // 7. AUTO-CLICKER (Shankara HTML Structure)
        function clickBet(predictedWinner) {
            ...
            const tryClick = setInterval(() => {
                ...
                // Determine the correct parent box
                ...
                if (container) {
                    ...
                    for (let row of rows) {
                        ...
                        // Ensure we are in the "Main" row
                        if (nameEl && nameEl.innerText.trim() === 'Main') {
                            // Verify the row is NOT suspended
                            if (!row.classList.contains('suspended-row')) {
                                const backBtn = row.querySelector('.casino-odds-box.back');
                                
                                // Double check the box itself isn't suspended
                                if (backBtn && !backBtn.classList.contains('suspended-box')) {
                                    clearInterval(tryClick); // Stop polling immediately
                                    
                                    // Slight randomized delay to mimic human reaction time
        ...
        updateEngineUI();
        console.log("%c SHANKARA TP SUITE ONLINE | AUTO-CLICKER ENGAGED ", "background:#000; color:#0f0; font-weight:bold; font-size:14px;");
    }
})();