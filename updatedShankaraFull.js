...
(function() {
    ...
    function runSuite() {
        ...
        // 4. LOGIC ENGINE (V5.0 OMNI-ENGINE WITH REGIME SWITCHING)
        
        // Sub-Engine Alpha (Weekday/Trend Rider)
        function engineAlpha(historyStr) {
            if(historyStr.length < 4) return "WAIT";
            const last3 = historyStr.slice(-3);
            const last4 = historyStr.slice(-4);
            const lastWinner = historyStr.slice(-1);

            const seq4 = historyStr.slice(-4);
            const occur4 = historyStr.lastIndexOf(seq4, historyStr.length - 5);
            if(occur4 !== -1) return historyStr[occur4 + 4] || "WAIT";

            if(last4 === "ABAB" || last4 === "BABA") return lastWinner === "A" ? "B" : "A"; 
            if(last3 === "AAA" || last3 === "BBB") return lastWinner;
            return "WAIT";
        }

        // Sub-Engine Beta (Weekend/Contrarian)
        function engineBeta(historyStr) {
            if(historyStr.length < 4) return "WAIT";
            const last3 = historyStr.slice(-3);
            const last4 = historyStr.slice(-4);

            if(last4 === "AAAA") return "B"; 
            if(last4 === "BBBB") return "A"; 
            if(last3 === "AAA") return "B"; 
            if(last3 === "BBB") return "A"; 
            if(last4 === "ABAB") return "B"; 
            if(last4 === "BABA") return "A"; 
            return "WAIT";
        }

        // The Virtual Regime Scanner
        function detectRegime(validHistory) {
            if(validHistory.length < 25) return { mode: "WARMUP", activeEngine: null, info: "COLLECTING DATA" };
            
            let alphaW = 0, alphaL = 0;
            let betaW = 0, betaL = 0;

            const testWindow = validHistory.slice(-25);
            for(let i = 5; i < testWindow.length; i++) {
                const histSlice = testWindow.slice(0, i).join('');
                const actualResult = testWindow[i];

                const pA = engineAlpha(histSlice);
                if(pA === actualResult) alphaW++;
                else if(pA !== "WAIT") alphaL++;

                const pB = engineBeta(histSlice);
                if(pB === actualResult) betaW++;
                else if(pB !== "WAIT") betaL++;
            }
            ...
            if(alphaRate >= 0.55 && alphaRate >= betaRate) {
                return { mode: "WEEKDAY (TREND)", activeEngine: engineAlpha, info: `A-EDGE: ${Math.round(alphaRate*100)}%` };
            }
            if(betaRate >= 0.55 && betaRate > alphaRate) {
                return { mode: "WEEKEND (FADE)", activeEngine: engineBeta, info: `B-EDGE: ${Math.round(betaRate*100)}%` };
            }
        ...
        function getDynamicPrediction(data) {
            ...
            const prediction = regime.activeEngine(historyStr);

            if(prediction === "WAIT") {
                return { win: "WAIT", reason: `NO SETUP IN ${regime.mode}` };
            }

            return { win: prediction, reason: `${regime.mode} | ${regime.info}` };
        }
        ...
        function updateEngineUI() {
            ...
            intelEl.innerHTML = `
                ...
                <div style="display:flex;justify-content:space-between;border-bottom:1px solid #333;padding-bottom:5px;margin-bottom:8px;">
                    <b style="color:#0ff;">SHANKARA TP (V5.0 OMNI)</b>
        ...
        function executeWager() {
            ...
            const fillSlip = setInterval(() => {
                ...
                if (stakeInput && submitBtn) {
                    ...
                    // Bypass React/Vue synthetic value setters
                    const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, "value").set;
                    nativeInputValueSetter.call(stakeInput, "250"); // Set stake amount
                    
                    // Force the UI framework to recognize the change
                    stakeInput.dispatchEvent(new Event('input', { bubbles: true }));
                    stakeInput.dispatchEvent(new Event('change', { bubbles: true }));

                    console.log(`%c [AUTO-BET] Stake injected. Waiting for Submit button to enable...`, "color:#0ff;");

                    // Wait slightly for framework state to update and remove the 'disabled' attribute
                    setTimeout(() => {
        ...
        function processHistoryArray(historyArray) {
            ...
            recentRounds.forEach(d => {
                ...
                if (!alreadyLogged) {
                    ...
                    // Require 25 hands for V5.0 virtual trading simulator
        ...
        updateEngineUI();
        console.log("%c SHANKARA TP SUITE ONLINE | V5.0 OMNI-ENGINE + AUTO-DECRYPT ENGAGED ", "background:#000; color:#0f0; font-weight:bold; font-size:14px;");
    }
})();
    ...
    function runSuite() {
        ...
        function updateEngineUI() {
            ...
            intelEl.innerHTML = `
                <style>@keyframes blink { 50% { opacity: 0; } }</style>
                <div style="display:flex;justify-content:space-between;border-bottom:1px solid #333;padding-bottom:5px;margin-bottom:8px;">
                    <b style="color:#0ff;">SHANKARA TP (ADVISOR V5.0)</b>
                    <span style="font-size:10px;color:#888;">${storage.percent}% DB</span>
                </div>
                <div style="text-align:center;font-size:10px;margin-bottom:5px;">
                    ${keyStatus}
        ...
        updateEngineUI();
        console.log("%c SHANKARA TP SUITE ONLINE | ADVISORY ONLY (NO AUTO-BET) ", "background:#000; color:#0f0; font-weight:bold; font-size:14px;");
    }
})();