(function () {
    ...
    window.tpStats = { wins: 0, losses: 0, units: 0 }; // Reset for backfill
    ...
    function calculateWin(cardStr) {
        ...
        const getScore = (hand) => {
            ...
            let isS = (hand[0].r === hand[1].r + 1 && hand[1].r === hand[2].r + 1) || (hand[0].r === 14 && hand[1].r === 3 && hand[2].r === 2);
            const tie = (hand[0].r * 10000) + (hand[1].r * 100) + hand[2].r;
            if (hand[0].r === hand[1].r && hand[1].r === hand[2].r) return 60000000 + tie;
            if (isS && hand[0].s === hand[1].s && hand[1].s === hand[2].s) return 50000000 + tie;
            if (isS) return 40000000 + tie;
            if (hand[0].s === hand[1].s && hand[1].s === hand[2].s) return 30000000 + tie;
    ...
    function getDynamicPrediction(historySlice) {
        ...
        // HYPER-STRICT REGIME GATES
        // 1. If it's chopping heavily, do not bet. The casino traps chop bettors.
        if (altScore >= 0.60) return { win: "WAIT", reason: "CHOP TRAP AVOIDED" };
        
        // 2. If it's pure noise (no clear direction), wait.
        if (altScore > 0.40 && maxStreak < 3) return { win: "WAIT", reason: "NOISE FILTER" };

        let res; let label;
        // Only trigger Markov 2 on undeniable trends
    ...
    function updateUI() {
        ...
        intelEl.innerHTML = `
            <b style="color:#0ff;">V8.7 HYPER-SNIPER (LORDS)</b><br>
            ...
            response.clone().json().then(json => {
                const traverse = (n) => {
                    ...
                    if (n.mid && n.card) {
                        ...
                        if (n.card.split(',').length === 6 && !n.card.includes("1")) {
    ...
    // INITIATE STATS RECALCULATION ON LOAD
    performBackfill();
    console.log("%c V8.7 HYPER-SNIPER LOADED ", "color:#0f0;font-weight:bold;");
})();
...
(function () {
    ...
    function updateUI() {
        ...
        intelEl.innerHTML = `
            <b style="color:#0ff;">V8.7 HYPER-SNIPER (VIEW-ONLY)</b><br>
            <div style="text-align:center;padding:10px;">
                <span style="font-size:24px;color:${activePrediction?.win === 'A' ? '#f44' : (activePrediction?.win === 'B' ? '#4af' : '#ff0')};font-weight:bold;">
                    ${winnerStr}
                </span><br>
    ...
    dlBtn.onclick = () => {
        ...
        const link = document.createElement("a");
        link.href = URL.createObjectURL(blob);
        link.download = `lords_v8_7_view.csv`;
        link.click();
    };
    container.appendChild(dlBtn);
    ...
    // INITIATE STATS RECALCULATION ON LOAD
    performBackfill();
    console.log("%c V8.7 HYPER-SNIPER VIEW-ONLY LOADED ", "color:#0f0;font-weight:bold;");
})();