...
(function () {
    ...
    dlBtn.onclick = () => {
        ...
        const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `shankara_v11_oracle.csv`; link.click();
    };
    ...
    function getOraclePrediction(h, mem) {
        ...
        // 1. Generate predictions from all engines
        const preds = {
            markov: getMarkov(h),
            diff: getDiff(h),
            bias: getBias(h),
        ...
        // 2. Calculate rolling 15-hand accuracy for each
        const getAcc = (arr) => {
        ...
        // 4. Decision Gate: Require 73% accuracy (11 out of 15 correct) to strike
        if (maxAcc >= 0.73 && bestEngine) {
            return { 
                win: preds[bestEngine], 
                reason: `${bestEngine.toUpperCase()} HOT (${Math.round(maxAcc*100)}%)`, 
                raw: preds 
            };
        }
    ...
    function updateEngineUI() {
        ...
        intelBox.innerHTML = `
            ...
            <b style="color:#0ff;">V11.0 THE ORACLE (VIEW)</b><br>
    ...
    performBackfill();
     console.log("%c V11.0 MULTI-ENGINE ORACLE ", "color:#0f0;font-weight:bold;");
})();
...
// bet placement
(function () {
    // Inject Crypto if Shankara needs it
    ...
    function runSuite() {
        ...
        // Dual-Platform Storage Keys (Adapts to Shankara or Lords automatically based on what's available)
        ...
        dlBtn.onclick = () => {
            const content = window.tpLogs.map(r => `${r.mid},${r.winner}`).join("\n");
            const blob = new Blob([content], { type: "text/plain" });
            const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `v11_oracle_auto.csv`; link.click();
        };
        container.appendChild(dlBtn);
        ...
        function getOraclePrediction(h, mem) {
            ...
            const preds = { markov: getMarkov(h), diff: getDiff(h), bias: getBias(h), fade: getFade(h) };
            ...
            const accs = { markov: getAcc(mem.markov), diff: getAcc(mem.diff), bias: getAcc(mem.bias), fade: getAcc(mem.fade) };
        ...
        function updateEngineUI() {
            ...
            intelBox.innerHTML = `
                <style>@keyframes blink { 50% { opacity: 0; } }</style>
                <b style="color:#0ff;">V11.0 ORACLE (AUTO-BET)</b><br>
                <div style="text-align:center;font-size:10px;">${keyS}</div>
                <div style="text-align:center;padding:10px;">
                    <span style="font-size:24px;color:${activePrediction?.win === 'A' ? '#f44' : (activePrediction?.win === 'B' ? '#4af' : '#ff0')};font-weight:bold;">
                        ${wStr}
        ...
        // --- SNIFFER (ONLY ACTIVE IF NOT LORDS) ---
        ...
        performBackfill();
        console.log("%c V11.0 THE ORACLE (UNIVERSAL AUTO-BET) LOADED ", "color:#0f0;font-weight:bold;");
    }
})();
