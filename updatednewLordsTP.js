...
(function() {
    ...
    // 3. LOGIC ENGINE (V5.0 OMNI-ENGINE WITH REGIME SWITCHING)
    ...
    function detectRegime(validHistory) {
        ...
        // Virtually trade the last 25 hands
        ...
        // Shift criteria: Requires 55%+ virtual edge to activate real bets
    ...
    function updateEngineUI() {
        ...
        intelEl.innerHTML = `
            <div style="display:flex;justify-content:space-between;border-bottom:1px solid #333;padding-bottom:5px;margin-bottom:8px;">
                <b style="color:#0ff;">1-DAY TP (V5.0 OMNI-ENGINE)</b>
    ...
    function processHistoryArray(historyArray) {
        ...
        recentRounds.forEach(d => {
            ...
            if (!alreadyLogged) {
                ...
                // Only record PNL if we actually bet something
    ...
    function processGameData(d) {
        ...
        if (mid !== activeMid) {
            ...
            // ARM AUTO-CLICKER (Only fires if win !== "WAIT")
            if (activePrediction && activePrediction.win !== "WAIT") {
    ...
    updateEngineUI();
    console.log("%c MASTER 1-DAY TP SUITE ONLINE | V5.0 OMNI-ENGINE ACTIVE ", "background:#000; color:#0f0; font-weight:bold; font-size:14px;");
})();
...
function updateEngineUI() {
    ...
    intelEl.innerHTML = `
        <div style="display:flex;justify-content:space-between;border-bottom:1px solid #333;padding-bottom:5px;margin-bottom:8px;">
            <b style="color:#0ff;">1-DAY TP (ADVISOR V5.0)</b>
            <span style="font-size:10px;color:#888;">${storage.percent}% DB</span>
        </div>
        <div style="text-align:center;margin-bottom:10px;">
            <span style="font-size:10px;color:#aaa;">PREDICTING MID: *${midStr}</span><br>
...
updateEngineUI();
console.log("%c MASTER 1-DAY TP SUITE ONLINE | V5.0 OMNI ADVISORY (NO AUTO-BET) ", "background:#000; color:#0f0; font-weight:bold; font-size:14px;");
})();