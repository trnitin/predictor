(function () {
    ...
    function runSuite() {
        ...
        /* ---------------- MARKOV-2 LOGIC ENGINE ---------------- */

        function computeMarkov2(history) {

            let AAA = 1, AAB = 1, ABA = 1, ABB = 1, BAA = 1, BAB = 1, BBA = 1, BBB = 1;
            ...
            return {
                AA: { A: AAA / (AAA + AAB), B: AAB / (AAA + AAB) },
                AB: { A: ABA / (ABA + ABB), B: ABB / (ABA + ABB) },
                BA: { A: BAA / (BAA + BAB), B: BAB / (BAA + BAB) },
                BB: { A: BBA / (BBA + BBB), B: BBB / (BBA + BBB) }
            };
        ...
        function getDynamicPrediction(data) {
            ...
            const markov = computeMarkov2(history);
            ...
            return {
                win: side,
                reason: `MARKOV2 ${(prob * 100).toFixed(1)}%`
            };
        ...
        /* ---- remaining script unchanged (UI updater, processors, decryptor, hooks, download, interval) ---- */
        // 6. UI UPDATER
        ...
        console.log("%c SHANKARA TP Markov 2 SUITE ONLINE | AUTO-DECRYPTION ENGAGED ", "background:#000;color:#0f0;font-weight:bold;font-size:14px;");
...
(function () {
    ...
    function runSuite() {
        ...
        /* ----- ADDITION: ORDER PLACEMENT SUITE (1:1 INTEGRATION) ----- */
        ...
        function updateEngineUI() {
            ...
            intelEl.innerHTML = `
                <style>@keyframes blink { 50% { opacity: 0; } }</style>
                <div style="display:flex;justify-content:space-between;border-bottom:1px solid #333;padding-bottom:5px;margin-bottom:8px;">
                    <b style="color:#0ff;">SHANKARA MARKOV2 (AUTO)</b>
                    <span style="font-size:10px;color:#888;">${storage.percent}% DB</span>
                </div>
                <div style="text-align:center;font-size:10px;margin-bottom:5px;">
                    ${keyStatus}
        ...
        btn.onclick = (e) => {
            ...
            const link = document.createElement("a");
            link.href = URL.createObjectURL(blob);
            link.download = `shankara_markov2_dataset_${Date.now()}.csv`;
            link.click();
        };
        ...
        console.log("%c SHANKARA MARKOV2 SUITE ONLINE | AUTO-BETTING ENGAGED ", "background:#000;color:#0f0;font-weight:bold;font-size:14px;");
