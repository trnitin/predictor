(function() {
    ...
    container.style = "position:fixed;top:20px;right:20px;left:0px;z-index:99999;display:flex;flex-direction:column;gap:8px;font-family:monospace;width:250px;";
    ...
    function executeWager() {
        ...
        // Stage 2: Wait for betslip to mount, fill stake, and submit
        const fillSlip = setInterval(() => {
            betslipAttempts++;
            if (betslipAttempts > 20) { // 2 second timeout for betslip UI
            ...
            // Ensure the elements exist and are physically visible on screen
            if (stakeInput && submitBtn && stakeInput.offsetParent !== null) {
                // Insert stake
                stakeInput.value = "25";
                
                // Dispatch framework events to validate the input
                stakeInput.dispatchEvent(new Event('input', { bubbles: true }));
                stakeInput.dispatchEvent(new Event('change', { bubbles: true }));

                // Add 200ms delay so frontend framework updates state before clicking place bet
                setTimeout(() => {
                    submitBtn.click();
    ...
    function clickBet(predictedWinner) {
        ...
        // Stage 1: Wait for odds to open
        const tryClick = setInterval(() => {
            attempts++;
            if (attempts > 20) { // 10 second timeout for odds
                ...
                if (nameTag && nameTag.innerText.trim() === targetName) {
                    if (!row.classList.contains('teenpatti-suspended')) {
                        ...
                        if (backBtn) {
                            backBtn.click();
                            clearInterval(tryClick);
                            console.log(`%c [AUTO-CLICK] Odds opened. Opening betslip...`, "color:#0f0;");
    ...
    updateEngineUI();
    console.log("%c MASTER 1-DAY TP SUITE ONLINE | FULL AUTOMATION ", "background:#000; color:#0f0; font-weight:bold; font-size:14px;");
})();