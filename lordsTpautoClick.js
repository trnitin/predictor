(function() {
    ...
    function clickBet(predictedWinner) {
        ...
        // Scan the DOM every 500ms for up to 10 seconds
        const tryClick = setInterval(() => {
            ...
            for (let row of rows) {
                ...
                if (nameTag && nameTag.innerText.trim() === targetName) {
                    // Betting is open if the suspended class is gone
                    if (!row.classList.contains('teenpatti-suspended')) {
                        const backBtn = row.querySelector('.back');
                        if (backBtn) {
                            backBtn.click();
                            clearInterval(tryClick);
                            console.log(`%c [AUTO-CLICK] Successfully clicked BACK for ${targetName}!`, "color:#0f0;font-weight:bold;");
                            return;
                        }
                    }
                }
    ...
    // 6B. LIVE ROUND PROCESSOR (With Click Trigger)
    ...
    updateEngineUI();
    console.log("%c MASTER 1-DAY TP SUITE ONLINE | AUTO-CLICKER ENABLED ", "background:#000; color:#0f0; font-weight:bold; font-size:14px;");
})();